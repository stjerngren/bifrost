...
torch::Tensor simulated_linear_forward(std::string layer_name, torch::Tensor input, torch::Tensor weight, std::string path_to_arch_file, std::string path_to_tile, float sparsity_level, bool transpose)
{
    ...
    if (transpose)
    {
        ...
        weight_changed = weight_changed.contiguous(); //Contigous is to transform the container so that it stores the data by the transposed rows.
    }
    ...
    int N = weight_changed.sizes()[1];      //Number of output neurons
    float *MK_input_raw = (float *)input.data_ptr();
    float *KN_weight_raw = (float *)weight_changed.data_ptr();
    //Creating output tensor
    torch::Tensor output;
    if (input_n_dim == 2)
    {
    ...
    else
    {
        ...
        simulateDenseGemmForward(layer_name, KN_weight_raw, MK_input_raw, output_raw, gemm_M, gemm_K, gemm_N, path_to_tile, stonne_cfg);
    }
