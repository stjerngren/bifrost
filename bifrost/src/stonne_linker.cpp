...
int simulateDenseConvForwardNHWC(std::string layer_name, float *input, float *weight, float *output, int R, int S, int C, int K, int G, int N, int X, int Y, int X_, int Y_, int strides, int pad_x, int pad_y, std::string path_to_tile, Config stonne_cfg)
{
    ...
    //Executing the accelerator
    Stonne *stonne_instance = new Stonne(stonne_cfg);
    stonne_instance->loadDNNLayer(CONV, layer_name, R, S, C, K, G, N, X + 2 * pad_x, Y + 2 * pad_y, strides, (address_t)input, (address_t)weight, (address_t)output, CNN_DATAFLOW);
    stonne_instance->loadTile(T_R, T_S, T_C, T_K, T_G, T_N, T_X_, T_Y_);
    stonne_instance->run(); //Running the accelerator and generates the output in ofmap_raw
    int cycles = stonne_instance->n_cycles;
    //Transform_Ofmap_Memory(ofmap_raw, output, K, X_, Y_); // Transform simulator memory format to caffe format.
...
int simulateSparseGemmForwardPsums(std::string layer_name, float *KN_matrix_raw, float *MK_matrix_raw, float *output_raw, int N, int G, int gemm_M, int gemm_K, int gemm_N, float sparsity_level, Config stonne_cfg, Dataflow dataflow)
{
...
int simulateSparseDenseGemm(std::string layer_name, float *MK_sparse_matrix, float *KN_dense_matrix, float *output_raw, int M, int K, int N, unsigned int *MK_col_id, unsigned int *MK_row_pointer, int T_N, int T_K, Config stonne_cfg)
{
...
int simulateDenseGemmForwardPsums(std::string layer_name, float *KN_matrix_raw, float *MK_matrix_raw, float *output_raw, int N, int G, int gemm_M, int gemm_K, int gemm_N, std::string path_to_tile, Config stonne_cfg)
{
    if (path_to_tile == "")
    {
        std::cout << "Tile file parameters must be specified" << std::endl;
...
int simulateDenseGemmForward(std::string layer_name, float *KN_matrix_raw, float *MK_matrix_raw, float *output_raw, int N, int G, int gemm_M, int gemm_K, int gemm_N, std::string path_to_tile, Config stonne_cfg)
{
    if (path_to_tile == "")
    {
        std::cout << "Tile file parameters must be specified" << std::endl;
