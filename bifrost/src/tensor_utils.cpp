...
float *Transform_Ifmap_Memory_b(const float *bottom_data, const int C, const int X, const int Y, const int pad_x, const int pad_y)
{
    ...
    for (int i = 0; i < n_channels; i++)
    {
        for (int y = 0; y < input_y; y++)
        {
            for (int x = 0; x < input_x; x++)
            {
                data_to_send[i * size_channel + y * (input_x) + x] = bottom_data[(n_channels * ((y + pad_y) * input_x_pad + x + pad_x)) + i];
            }
        }
    }
...
float *Transform_Ifmap_Memory_c(const float *bottom_data, const int C, const int X, const int Y, const int pad_x, const int pad_y)
{
    ...
    for (int i = 0; i < n_channels; i++)
    {
        for (int y = 0; y < input_y; y++)
        {
            for (int x = 0; x < input_x; x++)
            {
                data_to_send[(n_channels * ((y + pad_y) * input_x_pad + x + pad_x)) + i] = bottom_data[(n_channels * (y * input_x + x)) + i];
            }
        }
    }
    ...
    for (int n_f = 0; n_f < n_filters; n_f++)
    {
        for (int i = 0; i < n_channels; i++)
        {
            for (int y = 0; y < kernel_y; y++)
            {
                for (int x = 0; x < kernel_x; x++)
                {
                    filters_to_send[n_f * size_filter + i * size_channel + y * kernel_x + x] = weights[n_f * size_filter + (n_channels * (y * kernel_x + x)) + i];
                }
            }
        }
    }
