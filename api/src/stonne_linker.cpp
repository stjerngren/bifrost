...
float *Transform_Ifmap_Memory(const float *bottom_data, const int C,
                              const int X, const int Y, const int pad_x,
                              const int pad_y);

float *Transform_Filters_Memory(const float *weights, const int K, const int G,
                                const int C, const int R, const int S);

void Transform_Ofmap_Memory(const float *ofmap_data, float *top_data,
...
//    for j in h:
//        for k in w:
//            output[(c*(j+pad_w)*h*2*pad_h)+j] = input[i*h*w + j*h*k]
...
//    for j in w_out:
//        for k in h_out:
//            output[i*h*w+j*w_out+k] = input[o*(j*h_out)]
...
*/
  ...
  std::cout << "post conv" << std::endl;

  for (int i = 0; i < C * X * Y; i++) {
    std::cout << ifmap_to_send[i] << " ";
  }
  std::cout << std::endl;
  for (int i = 0; i < R * S * C; i++) {
  ...
  Transform_Ofmap_Memory(
      ofmap_raw, output, K, X_,
      Y_); // Transform simulator memory format to caffe format.
...
int simulateDenseConvForwardNHWC(std::string layer_name, float *input,
  ...
  stonne_instance->loadDNNLayer(
      CONV, layer_name, R, S, C, K, G, N, X + 2 * pad_x, Y + 2 * pad_y, strides,
      (address_t)input, (address_t)weight, (address_t)output, CNN_DATAFLOW);
  stonne_instance->loadTile(T_R, T_S, T_C, T_K, T_G, T_N, T_X_, T_Y_);
  stonne_instance
      ->run(); // Running the accelerator and generates the output in ofmap_raw
  int cycles = stonne_instance->n_cycles;
...
int simulateDenseConvForwardPsums(std::string layer_name, float *input,
  ...
  stonne_instance->loadDNNLayer(
      ...
      (address_t)ofmap_raw, CNN_DATAFLOW);
  stonne_instance->loadTile(T_R, T_S, T_C, T_K, T_G, T_N, T_X_, T_Y_);
  int psums = stonne_instance->mem->getPsums();
...
int simulateSparseGemmForward(std::string layer_name, float *KN_matrix_raw,
  ...
  for (int n = 0; n < N; n++) {
    ...
    for (int g = 0; g < G;
         g++) { // Every group is calculated with a different GEMM
      std::string layer_name_group =
          (layer_name_batch + ("_group_" + std::to_string(g)));
      float *pointer_MK_dense_matrix =
          (float *)MK_matrix_raw + weight_offset_ * g;
      ...
      if (dataflow == MK_STA_KN_STR) {
        ...
        MK_dense_matrix = generatePrunnedMatrix(pointer_MK_dense_matrix,
                                                gemm_M_grouped * gemm_K_grouped,
                                                sparsity_level);
        ...
        KN_dense_matrix = generatePrunnedMatrix(pointer_KN_dense_matrix,
                                                gemm_K_grouped * gemm_N_grouped,
                                                sparsity_level);
      ...
      unsigned int *acc_bitmap =
          new unsigned int[gemm_M_grouped *
                           gemm_N_grouped]; // Currently is not generated by the
                                            // accelerator

      // Generating bitmaps
      unsigned int *MK_bitmap = generateBitMapFromDense(
          MK_dense_matrix, gemm_M_grouped, gemm_K_grouped, GEN_BY_ROWS);
      unsigned int *KN_bitmap = generateBitMapFromDense(
          KN_dense_matrix, gemm_K_grouped, gemm_N_grouped, GEN_BY_COLS);

      // Generating sparse matrix
      float *MK_sparse_matrix = generateMatrixSparseFromDense(
          MK_dense_matrix, MK_bitmap, gemm_M_grouped, gemm_K_grouped,
          GEN_BY_ROWS);
      float *KN_sparse_matrix = generateMatrixSparseFromDense(
          KN_dense_matrix, KN_bitmap, gemm_K_grouped, gemm_N_grouped,
          GEN_BY_COLS);

      // Running STONNE
      Stonne *stonne_instance = new Stonne(stonne_cfg);

      stonne_instance->loadGEMM(
          layer_name_group, gemm_N_grouped, gemm_K_grouped, gemm_M_grouped,
          MK_sparse_matrix, KN_sparse_matrix, MK_bitmap, KN_bitmap, acc_output,
          acc_bitmap, dataflow); // Loading GEMM
      stonne_instance->run();

      // Add total number of cycles
      cycles = cycles + stonne_instance->n_cycles; // Running the simulator
      delete[] MK_bitmap;
      delete[] KN_bitmap;
...
int simulateSparseGemmForwardPsums(std::string layer_name, float *KN_matrix_raw,
  ...
  for (int n = 0; n < N; n++) {
    ...
    for (int g = 0; g < G;
      ...
      // Add total number of psums
      psums = psums + stonne_instance->mem->getPsums(); // Running the simulator
      delete[] MK_bitmap;
      delete[] KN_bitmap;
      delete[] MK_dense_matrix;
      delete[] KN_dense_matrix;
...
void *simulateSparseGemmWithBitmapsForward(
  ...
  for (int n = 0; n < N; n++) {
    ...
    for (int g = 0; g < G;
      ...
      unsigned int *pointer_MK_bitmap =
          (unsigned int *)MK_bitmap + weight_offset_ * g;
      unsigned int *pointer_KN_bitmap =
          (unsigned int *)KN_bitmap + col_offset_ * g;
      ...
      stonne_instance->loadGEMM(
          layer_name_group, gemm_N_grouped, gemm_K_grouped, gemm_M_grouped,
          MK_sparse_matrix, KN_sparse_matrix, pointer_MK_bitmap,
          pointer_KN_bitmap, acc_output, acc_bitmap, dataflow); // Loading GEMM
...
int simulateSparseDenseGemm(std::string layer_name, float *MK_sparse_matrix,
  ...
  stonne_instance->loadSparseDense(layer_name_batch, N, K, M, MK_sparse_matrix,
                                   KN_dense_matrix, MK_col_id, MK_row_pointer,
                                   output_raw, T_N, T_K); // Loading GEMM
  stonne_instance->run(); // Running the simulator
  return stonne_instance->n_cycles;
}
  ...
  for (int n = 0; n < N; n++) {
    ...
    for (int g = 0; g < G;
      ...
      float *pointer_MK_dense_matrix =
          (float *)MK_matrix_raw + weight_offset_ * g;
      float *pointer_KN_dense_matrix = (float *)KN_matrix_raw + col_offset_ * g;
      // Since the offset in this case is per column, we have to use a
      // submatrix.
      for (int i = 0; i < gemm_N_grouped; i++) {
        for (int j = 0; j < gemm_K_grouped; j++) {
          float *sub_pointer = (float *)KN_matrix_raw + col_offset_ * g;
          sub_KN_dense_matrix[i * gemm_K_grouped + j] =
              sub_pointer[i * gemm_K + j];
        }
      }
      ...
      Stonne *stonne_instance =
          new Stonne(stonne_cfg); // Creating the instance of the simulator
      stonne_instance->loadFCLayer(
          layer_name_group, gemm_M_grouped, gemm_K_grouped, gemm_N_grouped,
          (address_t)pointer_MK_dense_matrix,
          (address_t)pointer_KN_dense_matrix, (address_t)acc_output);
      stonne_instance->loadFCTile(T_K, T_M, T_N);
      psums = psums + stonne_instance->mem->getPsums();
...
int simulateDenseGemmForward(std::string layer_name, float *KN_matrix_raw,
  ...
  for (int n = 0; n < N; n++) {
    ...
    for (int g = 0; g < G;
      ...
      stonne_instance->run(); // Running the accelerator and generates the
                              // output in ofmap_raw
      cycles = cycles + stonne_instance->n_cycles;
      delete stonne_instance;
    }
  }
  delete[] sub_KN_dense_matrix;
