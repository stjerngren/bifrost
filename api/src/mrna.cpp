...
void mRNA_tiles(Stonne *stonne_instance, Config stonne_cfg, 
int X, int Y, int C, int R, int S, int X_, int Y_, int K, int N, int stride)
{
    std::cout << "Create MAERI" << std::endl;
    mrna::Maeri *maeri = new mrna::Maeri(stonne_cfg.m_MSNetworkCfg.ms_size,  stonne_cfg.m_SDMemoryCfg.n_read_ports,  stonne_cfg.m_SDMemoryCfg.n_write_ports);
    ...
    std::cout << "Get best" << std::endl;
     std::cout << "mymap.size() is " << analyzer->mappings.size() << '\n';
